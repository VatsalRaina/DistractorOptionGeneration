...
parser.add_argument('--context_path', type=str, help='Load path of contexts.txt')
parser.add_argument('--question_path', type=str, help='Load path of questions.txt (contains the answer two)')
...
def main(args):
    ...
    for item in test_data[start:end]:
        ...
        for generated_ids in all_generated_ids:
            ...
            all_generated_distractors.append(genQu)
            all_contexts.append(context.replace('\n', ' '))

    with open(args.save_path+str(args.part_num)+"_gen_distractors.txt", 'w') as f:
        f.writelines("%s\n" % qu for qu in all_generated_distractors)
